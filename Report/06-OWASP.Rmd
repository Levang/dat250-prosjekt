...
```python
try:
    accountSelected=int(transForm.accountSelect.data)
    if str(accountSelected) in str(accountList):
...
As seen in the example above, input validation is input validation is used to ensure query safety. We specifically require there to be no special characters and for general input fields, we use an illegal character checking function:

```python
#True means illegal character exists
