...
### Don’t store sensitive data unnecessarily.Discard it as soon as possible or use PCI DSS complianttokenization or even truncation. Data that is not retained cannot be sto len.
All sensitive data is only temporarily stored in redis with an expiry date. The data is otherwise at rest and stored in an encrypted form which is inaccessible without user input. This ensures that we do not store or have access to decrypted data longer than necessary.
...
The password stored in the database is generated with scrypt. We use the default settings as seen in the code snippet below. These settings in combination with our password length requirement of 12
we can rest assured that randomly bruteforcing these passwords would take longer than the years we have left to live with todays computers. 

#TODO Vidar kan du skrive litt her? En linje eller to idk...
