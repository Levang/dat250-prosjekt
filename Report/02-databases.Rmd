...
Nothing in the accounts table has been encrypted as it makes no difference to us whether a malicious party knows how much money is in a specific account. The don't know the owner, and will not be able to log in or do any attacks based on knowing the account number, and the associated balance. Had they been able to identify the owner, they could technically try to do a targeted attack on the owner of the account, but the rest of the security features on out website should be able to prevent that.
...
```
transactionID  accountFrom  accountTo  amountDB  message  time
```

- `transactionID` is a unique transaction id associated with the account. It simply iterates from 0 and up to a arbitrary number such that every transaction gets a unique ID.

- `accountFrom` is the account the transaction is made from.

- `accountTo` is the account to which the transaction is being sent.

- `amountDB` is the the amount that being sent from `accountFrom` to `accountTo`. 

- `message` is a message that gets sent along with the transaction. This could either be a normal message, or normally usually a KID-message.

- `time` is just a timestamp for the transaction.

As none of this data should be sensitive, nothing in the transactions table is encrypted. If someone got a hold of our database they could backtrack the transaction, but aside from knowing about something that has already gone through, this shouldn't affect our security.
